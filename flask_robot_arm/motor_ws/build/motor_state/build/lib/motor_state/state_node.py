...
def receive_can_messages(timeout=0.2):
    """持续接收 CAN 消息"""
    try:
...
class StateReader(Node):
    ...
    def timer_callback(self):
        send_can_message("can0", 0x001, [0x00])
        pkg=receive_can_messages(timeout=0.1)
        if pkg is not None:
            msg = Receive()
            msg.id = pkg['address']
            msg.state = pkg['status']
            msg.speed = pkg['speed']
            msg.position = pkg['angle']
            self.motor_publisher_.publish(msg)
            self.get_logger().info(f"{pkg}")
